/**
 * Select2 menempel dropdown ke `body` secara default; di dalam kolom NexaLayer
 * (`.nx-scroll` / `#nx_body_*`) wheel sering tidak menggulir daftar atau panel.
 * `dropdownParent` mengikat dropdown ke ancestor yang sama dengan area scroll.
 */
function nexaSelect2DropdownParentFromEditable(editableEl) {
  if (
    typeof window === "undefined" ||
    !window.jQuery ||
    !editableEl ||
    typeof editableEl.closest !== "function"
  ) {
    return undefined;
  }
  const host =
    editableEl.closest(".nx-scroll") ||
    editableEl.closest(".nx-card-body") ||
    editableEl.closest("[id^='nx_body_']");
  if (!host || host === document.body || host === document.documentElement) {
    return undefined;
  }
  try {
    return window.jQuery(host);
  } catch (_) {
    return undefined;
  }
}
...
class NexaField {
  ...
  getDisplayContainer(node) {
    if (!(node instanceof Element)) {
      return null;
    }

    return node.classList.contains("nexa-display-container") ? node : null;
  }
  ...
  editText(element) {
    ...
    if (inputType === "tags" || inputType === "tags-input") {
      ...
      // Bersihkan placeholder text jika ada
      const placeholder = element.getAttribute('data-placeholder');
      if (placeholder && originalText.trim() === placeholder) {
        originalText = '';
        element.textContent = '';
        element.removeAttribute('data-is-placeholder');
      }
    }
    ...
    const displayContainer = this.getDisplayContainer(elementParent);
    ...
    if (inputType === "tags") {
      ...
      if (displayContainer) {
        ...
        elementParent.replaceChild(inputElement, element);
      }
    } else if (inputType === "tags-input") {
      ...
      if (inputType !== "select" && inputType !== "radio" && inputType !== "textarea") {
        inputElement.select();
      }
    }
    ...
    const saveEdit = () => {
      ...
      if (newText === originalText.trim()) {
        ...
        if (inputType !== "tags" && inputType !== "tags-input" && inputElement.parentNode) {
          ...
          if (inputType === "tags-input" && inputElement.parentNode) {
            ...
            const displayContainer = this.getDisplayContainer(inputParent);
            
            if (displayContainer) {
              displayContainer.replaceChild(element, inputElement);
            } else if (inputParent) {
              inputParent.replaceChild(element, inputElement);
            }
          }
        }
        this.resetEditingState();
      ...
      if (this.validateInput(newText, inputType, element)) {
        ...
        if (inputType !== "tags" && inputType !== "tags-input") {
          if (newText === '') {
            // Kembalikan placeholder jika nilai dikosongkan
            const placeholder = element.getAttribute('data-placeholder');
            if (placeholder) {
    ...
    // Untuk radio, gunakan change event langsung (plain select)
    if (inputType === "radio") {
  ...
  forceSaveOrCancel() {
    ...
    if (
      ...
      if (
        ...
        try {
          if (document.body.contains(this.currentInputElement)) {
      ...
      // Cek apakah input ada dalam display container
      const currentInputParent = this.currentInputElement.parentElement;
      const displayContainer = this.getDisplayContainer(currentInputParent);
      ...
      } else if (currentInputParent) {
        // Tidak ada display container, ganti input dengan element langsung
        currentInputParent.replaceChild(
          this.currentEditingElement,
          this.currentInputElement
        );
      }
  ...
  // Method untuk toggle nilai checkbox/switch (true/false)
  handleToggle(element) {
    ...
    const isTrue = currentText === "true" || currentText === "1" || currentText === "yes";
    ...
    if (this.onSave) {
      this.onSave(element.id, newValue, element, inputType, fieldName);
    }
  }
  ...
  createTextInput(inputType, originalText) {
    ...
    input.value = typeof originalText === 'string' ? originalText.trim() : originalText;
  ...
  createSelectInput(element, originalText) {
    ...
    setTimeout(() => {
      if (window.NXUI && window.NXUI.initSelect2) {
        ...
        const dropdownParent = nexaSelect2DropdownParentFromEditable(element);
  ...
  // Method untuk membuat radio input (plain select tanpa Select2)
  ...
  initElements(selector = ".editable") {
    ...
    elements.forEach((element) => {
      ...
      // Alignment khusus untuk type number pada display element
      if (type === "number") {
      ...
      if (!isDisabled && !isReadonly) {
        if (type === "checkbox" || type === "switch") {
          ...
          element.addEventListener("click", () => this.handleToggle(element));
        } else {
          element.style.cursor = "pointer";
      ...
      // Tampilkan placeholder jika nilai kosong
      if (type !== "checkbox" && type !== "switch" && type !== "tags" && type !== "tags-input") {
        const placeholder = element.getAttribute('data-placeholder');
  ...
  addLabelToElement(element) {
    ...
    if (
      ...
      !this.getDisplayContainer(element.parentElement)
    ) {
