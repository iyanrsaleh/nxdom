...
export function Storage() {
 ...
 package: function (packageName) {
  ...
  proxy.cloud = function() {
    ...
    return {
      /**
       * Upload avatar — POST ke /upload/avatar → PHP cloud/avatar
      ...
      avatar: async function(options = {}) {
        ...
        return _postFormData('/upload/avatar', fd);
      },

      /**
       * Upload file umum — POST ke /upload/file → PHP cloud/add
       * @param {object} [options]
      ...
      add: async function(options = {}) {
        ...
        if (options.file instanceof File) {
          files = [options.file];
        } else if (Array.isArray(options.file)) {
          files = options.file.filter(f => f instanceof File);
        } else {
        ...
        fd.append('php_endpoint',  'cloud/add');
        if (field) fd.append('field', String(field));

        return _postFormData('/upload/file', fd);
      },
      ...
      Background: function(options = {}) {
        ...
        // Jika file sudah ada, langsung upload
        if (options.file instanceof File) {
          return _doUpload([options.file]);
        }
        if (Array.isArray(options.file) && options.file.length > 0) {
          return _doUpload(options.file.filter(f => f instanceof File));
        }
      ...
      /**
       * Update file pada record existing — POST ke /upload/file → PHP cloud/update
      ...
      update: async function(options = {}) {
        ...
        fd.append('php_endpoint', 'cloud/update');
