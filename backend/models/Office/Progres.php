 ...
 */
class Progres extends NexaModel
{
    private $formula;
    ...
    public function NestedTabel(array $bulder): array {
      try {
        ...
        $sql = implode(" ", array_filter([$select, $from, $where, $groupBy, $orderBy, $limit]));
                ...
                // Optimize output: add percentage calculation for all results
                if (!empty($result)) {
 ...
 */  
    ...
    private function optimizeCountOutput(array $result, array $groupByFields): array {
        ...
        // Check if result already has the expected structure (label, total, etc.)
        $firstRow = reset($result);
        if (isset($firstRow['label']) && isset($firstRow['total'])) {
            // Data already in the expected format, just add percentage
            $totalSum = array_sum(array_column($result, 'total'));
            
            $optimized = [];
            foreach ($result as $item) {
                // Skip items with null label
                if ($item['label'] === null || $item['label'] === '') {
                    continue;
                }
                
                $percentage = $totalSum > 0 ? round(($item['total'] / $totalSum) * 100, 2) : 0;
                $progres = $totalSum > 0 ? round(($item['total'] / $totalSum) * 100) : 0;
                
                $optimized[] = [
                    'label' => $item['label'],
                    'name' => $groupByFields[0] ?? 'unknown',
                    'total' => $item['total'],
                    'percent' => $percentage,
                    'progres' => $progres
                ];
        ...
        // Original logic for complex data structure
        $fieldGroups = [];
        ...
        // Calculate total for each field and add percentage
        $optimized = [];
        foreach ($fieldGroups as $fieldName => $fieldData) {
            // Calculate total count for this field
            $fieldTotal = array_sum(array_column($fieldData, 'value'));
            ...
            foreach ($fieldData as $item) {
                ...
                $percentage = $fieldTotal > 0 ? round(($item['value'] / $fieldTotal) * 100, 2) : 0;
                $progres = $fieldTotal > 0 ? round(($item['value'] / $fieldTotal) * 100) : 0;
