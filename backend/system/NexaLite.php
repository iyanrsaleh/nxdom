...
use PDO;
/**
 * NexaLite - SQLite Database ORM
 * 
 * Class ini adalah versi SQLite dari NexaModel.
 * Gunakan class ini untuk bekerja dengan database SQLite.
 * 
 * @package App\System
 * @see NexaModel Untuk versi MySQL
 */
class NexaLite {
    private $db;
    private $table;
    ...
    // Security & Performance Properties - SQLite compatible functions
    private $allowedFunctions = ['UPPER', 'LOWER', 'DATE', 'YEAR', 'MONTH', 'DAY', 'COUNT', 'SUM', 'AVG', 'MAX', 'MIN', 'RANDOM', 'RAND', 'CONCAT', 'GROUP_CONCAT', 'SUBSTR', 'SUBSTRING', 'TRIM', 'LTRIM', 'RTRIM', 'LENGTH', 'COALESCE', 'IFNULL', 'NULLIF', 'STRFTIME', 'DATETIME', 'DATE', 'TIME', 'JULIANDAY', 'FORMAT', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END', 'IIF', 'IF', 'CURRENT_DATE', 'CURRENT_TIME', 'CURRENT_TIMESTAMP', 'DATETIME', 'STRFTIME', 'UNIXEPOCH', 'ROUND', 'CEIL', 'FLOOR', 'ABS', 'SIGN', 'MOD', 'POWER', 'POW', 'SQRT', 'LOG', 'LOG10', 'EXP', 'MAX', 'MIN', 'REPLACE', 'REVERSE', 'SUBSTR', 'HEX', 'LOWER', 'UPPER', 'LENGTH', 'TYPEOF', 'CAST', 'JSON_EXTRACT', 'JSON_ARRAY', 'JSON_OBJECT', 'JSON_GROUP_ARRAY', 'JSON_GROUP_OBJECT', 'JSON_VALID', 'JSON_QUOTE', 'JSON_UNQUOTE', 'INSTR', 'LIKE', 'GLOB', 'MATCH'];
    ...
    public function __construct() {
        try {
            // Use SQLite connection instead of MySQL
            $nexaDb = NexaDb::getInstance();
            $this->db = $nexaDb->connSQLite();
            if (!$this->db) {
                $this->logError("Failed to get SQLite database instance", null, []);
                throw new \Exception("Failed to get SQLite database instance");
            }
            ...
            $this->logInfo("NexaLite (SQLite) initialized successfully");
        } catch (\Exception $e) {
            $this->logError("NexaLite (SQLite) initialization error: " . $e->getMessage(), null, []);
            throw new \Exception("Failed to initialize SQLite database connection: " . $e->getMessage(), 0, $e);
        }
    }
    ...
    private function ensureConnection() {
        try {
            if (!$this->db) {
                ...
                if (!$this->db) {
                    $this->logError("SQLite connection test failed", null, []);
                    throw new \Exception("SQLite connection test failed");
                }
                $this->logInfo("SQLite connection established successfully");
            }
        } catch (\Exception $e) {
            $this->logError("SQLite connection error: " . $e->getMessage(), null, []);
    ...
    private function logError($message, $query = null, $bindings = []) {
        ...
        error_log("NexaLite Error: " . json_encode($context));
    }
    ...
    private function validateFunctionParameters($functionCall) {
        ...
        // Special validation for specific functions (SQLite compatible)
        switch ($function) {
            case 'STRFTIME':
            case 'DATE_FORMAT': // Support both for compatibility
    ...
    /**
     * Validate STRFTIME/DATE_FORMAT function parameters (SQLite compatible)
     */
    private function validateDateFormatParams($params) {
        // STRFTIME(format, date) or DATE_FORMAT(date, format) - should have exactly 2 parameters
        $paramArray = $this->splitFunctionParams($params);
        if (count($paramArray) !== 2) {
            throw new \InvalidArgumentException("STRFTIME/DATE_FORMAT requires exactly 2 parameters");
        }
        
        // First parameter should be a column name or date expression
        $dateParam = trim($paramArray[0], " '\"");
        if (!preg_match('/^[a-zA-Z0-9_\.()]+$/', $dateParam)) {
            throw new \InvalidArgumentException("Invalid date parameter in STRFTIME/DATE_FORMAT: $dateParam");
        }
        
        // Second parameter should be a format string (SQLite uses %Y, %m, %d, etc.)
        $formatParam = trim($paramArray[1], " '\"");
        if (!preg_match('/^[%a-zA-Z0-9\-\/\:\s\.\,\(\)]+$/', $formatParam)) {
            throw new \InvalidArgumentException("Invalid format parameter in STRFTIME/DATE_FORMAT: $formatParam");
        }
    ...
    /**
     * Insert or update on duplicate key (SQLite compatible)
    ...
    public function upsert(array $data, array $updateColumns = []) {
        ...
        // SQLite uses INSERT ... ON CONFLICT ... DO UPDATE syntax
        $query = "INSERT INTO {$this->table} ($columns) VALUES ($values)";
        
        if (!empty($updateColumns)) {
            $updates = [];
            foreach ($updateColumns as $column) {
                // SQLite uses excluded.column_name instead of VALUES(column)
                $updates[] = "$column = excluded.$column";
            }
            $query .= " ON CONFLICT(id) DO UPDATE SET " . implode(', ', $updates);
        } else {
     ...
     */
            public function showTables() {
                $this->ensureConnection();
                // Ambil semua tabel dari SQLite database menggunakan sqlite_master
    ...
    private function getTableColumnNames($tableName = null) {
        try {
            ...
            // Use SQLite PRAGMA instead of SHOW COLUMNS
            ...
            $columnNames = [];
            foreach ($columns as $column) {
                $columnNames[] = $column['name']; // SQLite uses 'name' instead of 'Field'
            }
    ...
    public function getTablesInfo() {
        $this->ensureConnection();
        // Use SQLite sqlite_master instead of INFORMATION_SCHEMA
        $query = "
            SELECT 
                name,
                0 as rows, -- SQLite doesn't store row count directly
                0 as data_length, -- SQLite doesn't provide this easily
                0 as index_length, -- SQLite doesn't provide this easily
                '' as comment, -- SQLite doesn't support table comments
                'sqlite' as engine,
                '' as collation -- SQLite doesn't use collation like MySQL
            FROM sqlite_master 
        ...
        // Try to get actual row counts for each table
        foreach ($tables as &$table) {
            try {
                $countStmt = $this->db->query("SELECT COUNT(*) as cnt FROM `{$table['name']}`");
                $countResult = $countStmt->fetch(\PDO::FETCH_ASSOC);
                $table['rows'] = (int)$countResult['cnt'];
            } catch (\Exception $e) {
                $table['rows'] = 0;
            }
        }
    ...
    public function getTableColumns($tableName) {
        $this->ensureConnection();
        // Use SQLite PRAGMA instead of INFORMATION_SCHEMA
        $query = "PRAGMA table_info(`{$tableName}`)";
        ...
        // Map SQLite PRAGMA results to match MySQL INFORMATION_SCHEMA format
        $result = [];
        foreach ($columns as $col) {
            $result[] = [
                'name' => $col['name'],
                ...
                'max_length' => null, // SQLite doesn't store this in PRAGMA
    ...
    public function getDatabaseInfo() {
        ...
        $info = [
            ...
            'total_tables' => 0,
            'database_size' => 0,
            'charset' => 'UTF-8', // SQLite uses UTF-8 by default
            'collation' => '' // SQLite doesn't use collation like MySQL
        ];
        
        // Get database name from connection (SQLite file path)
        try {
            $dbPath = $this->db->query("PRAGMA database_list")->fetchAll(\PDO::FETCH_ASSOC);
            if (!empty($dbPath)) {
                $info['database_name'] = basename($dbPath[0]['file'] ?? 'database.sqlite');
            } else {
                $info['database_name'] = 'database.sqlite';
            }
        } catch (\Exception $e) {
            $info['database_name'] = 'database.sqlite';
        }
        
        // Get database info from sqlite_master
        $query = "
            SELECT 
        ...
        try {
            $dbPath = $this->db->query("PRAGMA database_list")->fetchAll(\PDO::FETCH_ASSOC);
            if (!empty($dbPath) && isset($dbPath[0]['file'])) {
                $filePath = $dbPath[0]['file'];
                if (file_exists($filePath)) {
                    $info['database_size'] = filesize($filePath);
                }
            }
        } catch (\Exception $e) {
    ...
    public function tablesRetInsert(array $tableAlias, array $data): array
    {
        ...
        try {
            ...
            // Hanya isi kolom timestamp bila ada di skema (SQLite PRAGMA) — sama perilaku NexaModel (MySQL)
