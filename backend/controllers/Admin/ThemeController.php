...
/**
 * ThemeController - Upload & kelola theme/template via aplikasi
 *
 * Upload file theme ketika aplikasi sudah online di server,
 * tanpa perlu akses langsung (SSH/FTP).
 * Layout dashboard tidak lewat sini — gunakan DistroController.
 *
 * @package App\Controllers\Admin
 */
class ThemeController extends NexaController
{
    private const ALLOWED_LAYOUTS = ['theme', 'mobile', 'tablet', 'docs'];
    ...
    /**
     * Halaman index - form upload theme
     * Mengikuti pola PostinganController/AccountController: hanya set data,
     * AdminController yang menangani template path & render.
     */
    ...
    /**
     * Upload single file .html (dipanggil via /{username}/theme/uploadFile)
     */
    public function uploadFile(array $params = []): void
    {
        ...
        try {
            ...
            error_log('ThemeController uploadFile: ' . $e->getMessage());
            $this->jsonResponse(['success' => false, 'message' => $e->getMessage(), 'error' => 'EXCEPTION']);
        }
    }

    /**
     * Upload & ekstrak ZIP theme (dipanggil via /{username}/theme/uploadZip)
     */
    public function uploadZip(array $params = []): void
    {
        ...
        try {
            ...
            $layout = $this->sanitizeLayout($_POST['layout'] ?? 'theme');
            ...
            for ($i = 0; $i < $zip->numFiles; $i++) {
                ...
                if (!in_array($ext, $allowedInZip)) continue;
            ...
            error_log('ThemeController uploadZip: ' . $e->getMessage());
            $this->jsonResponse(['success' => false, 'message' => $e->getMessage(), 'error' => 'EXCEPTION']);
        }
    }
    ...
    /**
     * Normalisasi path di dalam ZIP: dinamis sesuai isi file ZIP.
     * Struktur apa pun di dalam ZIP (file di root, folder, subfolder, nested) tetap dipertahankan persis.
     * - File di root: index.html -> layout/index.html
     * - Satu folder: example/css/file.css -> layout/example/css/file.css
     * - Nested: a/b/c/file.html -> layout/a/b/c/file.html
     * Hanya membersihkan: backslash, path traversal (..), null byte.
     */
