...
var JsonViewer = class extends h3 {
  ...
  *search(criteria) {
                    ...
                    : ke`
                              ...
                              >
                                  ...
                                  >
                                      ${key}:
                                      ${nn(
                                        !isPrimitiveNode && !isExpanded,
                                        () => this.renderNodePreview(nodeData)
                                      )}
                                  </span>

                                  ${nn(isPrimitiveNode || isExpanded, () =>
  ...
  renderNodePreview(node) {
    return ke`<span part="preview" class="preview"> ${generateNodePreview(
      node
    )} </span>`;
  }
  renderPrimitive(node, path) {
    ...
    const primitiveNode = ke`
            <span part="primitive primitive-${nodeType}" class="${getType(
      node
    )}"> ${renderedValue} </span>
        `;
...
// Export the JsonViewer class and define custom element
