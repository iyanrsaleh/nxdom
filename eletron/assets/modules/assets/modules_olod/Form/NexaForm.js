 ...
 console.log('label:', validation);
  ...
  if (data.onclick && data.onclick.send) {
    ...
    if (data.onclick.buttonsFull) {
      formFooter.style.cssText = "display: flex; justify-content: space-between; align-items: center; padding: 0;";
    } else {
      formFooter.style.cssText = "display: flex; justify-content: space-between; align-items: center; padding: 10px;";
    }
    ...
    cancelButton.className = data.onclick.cancelClass || "btn btn-secondary";
    if (data.onclick.buttonsFull) cancelButton.style.flex = "1";
    cancelButton.textContent = typeof data.onclick.cancel === 'string' ? data.onclick.cancel : "Cancel";
