...
class NexaModels {
  static DEFAULT_STORAGE_NAME = "nexa";
  static DEFAULT_INTERNAL_CONNECTIONS = {
    mysql2: {
      host: "localhost",
      user: "root",
      password: "",
    ...
    sqlite3: {
      filename: "userData:nexa.sqlite",
    },
    websocket: {
      url: "ws://127.0.0.1:4117",
      backendMode: "sqlite3",
    },
  };
   ...
   */
  reset() {
    this.tableName = NexaModels.DEFAULT_STORAGE_NAME;
  ...
  mysql2(connection = null) {
    this.transportMode = "mysql2";
   ...
   */
  Storage(table) {
    const target = String(table ?? NexaModels.DEFAULT_STORAGE_NAME).trim() || NexaModels.DEFAULT_STORAGE_NAME;
  ...
  clone() {
    ...
    cloned.transportMode = this.transportMode;
    cloned.connectionConfig = this.connectionConfig ? { ...this.connectionConfig } : null;
  ...
  static createInternalTransport(options = {}) {
    const bridgeName = String(options.bridgeName || "electronAPI");
    const invokeMethod = String(options.invokeMethod || "awaitDispatch");
    const channel = String(options.channel || "nexa-await-dispatch");
    const source = typeof globalThis !== "undefined" ? globalThis : {};
    const bridge = source?.[bridgeName];
    ...
    return {
      post: async (endpoint, payload) => {
        ...
        if (invokeMethod === "invoke") {
          return await bridge[invokeMethod](channel, packet);
        }
        return await bridge[invokeMethod](packet);
      },
    };
  }

  static setTransport(transport) {
    NexaModels.transport = transport;
  }

  static getTransport() {
    if (NexaModels.transport) return NexaModels.transport;
    if (typeof globalThis !== "undefined" && globalThis.NEXA?.awaitTransport) {
      return globalThis.NEXA.awaitTransport;
    }
    const internalTransport = NexaModels.createInternalTransport();
    if (internalTransport) return internalTransport;
    return null;
  }

  async dispatch(endpoint, payload) {
    const transport = NexaModels.getTransport();
    if (!transport) {
      throw new Error(
        "NexaAwait transport belum diset. Gunakan NXUI.Await.setTransport(...) atau NEXA.awaitTransport."
      );
    }
    ...
    if (
      ...
      normalizedPayload.mode = this.transportMode || "sqlite3";
    }
    if (
      normalizedPayload &&
      typeof normalizedPayload === "object" &&
      String(normalizedPayload.mode || "").toLowerCase() === "mysql2" &&
      !normalizedPayload.connection
    ) {
      normalizedPayload.connection = {
        ...NexaModels.DEFAULT_INTERNAL_CONNECTIONS.mysql2,
      };
    }
    if (
      normalizedPayload &&
      typeof normalizedPayload === "object" &&
      String(normalizedPayload.mode || "").toLowerCase() === "sqlite3" &&
      !normalizedPayload.connection
    ) {
      normalizedPayload.connection = {
        ...NexaModels.DEFAULT_INTERNAL_CONNECTIONS.sqlite3,
      };
    }
    if (
      normalizedPayload &&
      ...
      typeof this.connectionConfig === "object"
    ) {
      normalizedPayload.connection = {
        ...(normalizedPayload.connection || {}),
        ...this.connectionConfig,
    ...
    if (typeof transport === "function") {
      return await transport({ endpoint, payload: normalizedPayload, model: this });
    }
    if (transport && typeof transport.post === "function") {
      return await transport.post(endpoint, normalizedPayload);
    }
    throw new Error("NexaAwait transport tidak valid. Wajib function atau objek dengan method post().");
  }
  ...
  async get(endpoint = "Fetch", options = {}) {
    const sqlPlain = this.toSql();
    const payload = {
      sql: await this.nexaEncrypt.encryptJson(sqlPlain),
      sqlPlain,
    ...
    return await this.dispatch(endpoint, payload);
  }
  ...
  async insert(data, endpoint = "Fetch", options = {}) {
    const sqlPlain = this.insertSql(data);
    const payload = {
      sql: await this.nexaEncrypt.encryptJson(sqlPlain),
      sqlPlain,
      table: await this.nexaEncrypt.encryptJson(this.tableName),
  ...
  async upload(data, endpoint = "Fetch", options = {}) {
    const payload = {
  ...
  realtime(onData, onError = null, options = {}) {
    if (this.transportMode !== "websocket") {
      throw new Error("realtime() hanya untuk mode websocket. Gunakan .websocket() sebelum realtime().");
    }
    if (typeof onData !== "function") {
      throw new Error("realtime() membutuhkan callback onData(result).");
    }

    const connection = {
      ...(NexaModels.DEFAULT_INTERNAL_CONNECTIONS.websocket || {}),
      ...(this.connectionConfig || {}),
      ...(options || {}),
    };
    const wsUrl = String(connection.url || "ws://127.0.0.1:4117");
    const ws = new WebSocket(wsUrl);

    ws.onopen = () => {
      ws.send(
        JSON.stringify({
          type: "subscribe-select",
          backendMode: String(connection.backendMode || "sqlite3"),
          query: {
            type: "select",
            sqlPlain: this.toSql(),
            bindings: this.getBindings(),
  ...
  async update(data, endpoint = "Fetch", options = {}) {
    const sqlPlain = this.updateSql(data);
    const payload = {
      sql: await this.nexaEncrypt.encryptJson(sqlPlain),
      sqlPlain,
      table: await this.nexaEncrypt.encryptJson(this.tableName),
  ...
  async paginate(page = 1, perPage = 10, endpoint = "Fetch", options = {}) {
    ...
    const payload = {
      ...
      countSqlPlain: pagination.countSql,
      dataSql: await this.nexaEncrypt.encryptJson(pagination.dataSql),
      dataSqlPlain: pagination.dataSql,
      table: await this.nexaEncrypt.encryptJson(this.tableName),
      bindings: this.getBindings(),
      type: "paginate",
      page: page,
  ...
  async count(column = "*", endpoint = "Fetch", options = {}) {
    const sqlPlain = this.countSql(column);
    const payload = {
      sql: await this.nexaEncrypt.encryptJson(sqlPlain),
      sqlPlain,
      table: await this.nexaEncrypt.encryptJson(this.tableName),
