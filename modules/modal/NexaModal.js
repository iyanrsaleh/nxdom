...
class NexaModal {
  ...
  open(modalId, data = null) {
    return new Promise((resolve, reject) => {
      ...
      // Re-initialize Select2 AFTER modal is visible (display:block + show class).
      // Must destroy first — previous init ran while modal was display:none so
      // width calculation was wrong and rendered as native select.
      // IMPORTANT: use element reference (not #id selector) to avoid affecting
      // outside forms that share the same field names/IDs.
      const _modalForSelect2 = modal;
      setTimeout(() => {
        const allSelects = _modalForSelect2.querySelectorAll(".form-group-body select");
        if (allSelects.length === 0) return;

        const doReinit = (attemptsLeft) => {
          if (!(window.$ && window.$.fn && window.$.fn.select2)) {
            if (attemptsLeft > 0) setTimeout(() => doReinit(attemptsLeft - 1), 300);
            return;
          }
          allSelects.forEach((sel) => {
            try {
              // Use element reference directly — NOT #id selector, to avoid
              // accidentally reinit-ing a same-named select on an outside form.
              const $sel = window.$(sel);
              try { if ($sel.data('select2')) { $sel.select2('destroy'); } } catch(e) {}
              sel.removeAttribute('data-select2-initialized');
              $sel.select2({
                width: '100%',
                allowClear: true,
                placeholder: sel.querySelector('option[value=""]')?.textContent || 'Pilih opsi...',
              ...
              sel.setAttribute('data-select2-initialized', 'true');
            } catch (e) { /* silent */ }
          });
        };
        doReinit(10);
...
export async function modalHTML(data) {
  ...
  if (data.wizard) {
    ...
    // Gunakan render() agar formElement di-append langsung ke DOM (isConnected=true)
    // sehingga waitForSelect2AndInitialize bisa menginisialisasi Select2 dengan benar.
    template.render(modalBody);
  } else {
    modalBody.innerHTML = data.content ?? "";
  }
  // Inject toast info jika disediakan
  if (data.toastInfo?.show) {
    const toastWrapper = document.createElement('div');
    toastWrapper.style.width = '100%';
    toastWrapper.style.marginTop = '16px';
    toastWrapper.innerHTML = `<div class="Toast"><div class="Toast-border"><span class="Toast-icon" aria-hidden="true"><span class="material-symbols-outlined Toast-iconSymbol">info</span></span><div class="Toast-content">${data.toastInfo.text || ''}</div><button class="Toast-dismissButton" type="button" aria-label="Tutup notifikasi" onclick="NexaModalScreen.dismissToastInfo('${data.toastInfo.key}', this)">✕</button></div></div>`;
    modalBody.appendChild(toastWrapper);
  }
  ...
  if (data.onclick && data.onclick.send && !data.wizard) {
    ...
    // Add custom footer content if specified (raw HTML)
    if (data.footer) {
      customFooterContainer.innerHTML = data.footer;
    }

    // Add structured footer buttons if specified
    if (Array.isArray(data.footerButtons) && data.footerButtons.length > 0) {
      const modalKey = data.setDataBy?.modalKey || '';
      data.footerButtons.forEach(({ label, cls = 'secondary', method }) => {
        if (!label || !method) return;
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = `btn btn-${cls}`;
        btn.textContent = label;
        btn.setAttribute('onclick', `(typeof nx['${method}']==='function'?nx['${method}']:typeof nx['${label}']==='function'?nx['${label}']:function(){console.warn('nx.${method} dan nx.${label} tidak ditemukan')})('${modalKey}')`);
        customFooterContainer.appendChild(btn);
      });
    }
    ...
    // Render icon + label jika submitIcon diisi
    const submitIcon = String(data.onclick?.submitIcon || '').trim();
    const submitIconType = String(data.onclick?.submitIconType || 'octicon').trim();
    const submitTitle = data.onclick.title || "Save";
    if (submitIcon) {
      let iconEl;
      if (submitIconType === 'awesome') {
        // Font Awesome: <i class="fa-solid fa-archive">
        iconEl = document.createElement('i');
        iconEl.className = submitIcon.startsWith('fa-') ? `fa-solid ${submitIcon}` : submitIcon;
        iconEl.style.cssText = 'margin-right:var(--base-size-4,4px);vertical-align:text-bottom;font-size:1em;';
      } else if (submitIconType === 'material') {
        // Material Symbols: <span class="material-symbols-outlined">archive</span>
        iconEl = document.createElement('span');
        iconEl.className = 'material-symbols-outlined';
        iconEl.textContent = submitIcon;
        iconEl.style.cssText = 'margin-right:var(--base-size-4,4px);vertical-align:text-bottom;font-size:1em;line-height:1;';
      } else {
        // Octicon (default): <span class="octicon octicon-archive-16">
        // margin-right & vertical-align dihandle oleh .btn .octicon di buttons/index.css
        iconEl = document.createElement('span');
        iconEl.className = submitIcon.startsWith('octicon') ? submitIcon : `octicon ${submitIcon}`;
      }
      saveButton.appendChild(iconEl);
      saveButton.appendChild(document.createTextNode(submitTitle));
    } else {
      saveButton.textContent = submitTitle;
    }
