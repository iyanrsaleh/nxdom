/** true jika dark mode aktif (class di html atau body). */
function nexaTagsIsDarkMode() {
  try {
    return (
      document.documentElement.classList.contains("dark-mode-grid") ||
      document.body.classList.contains("dark-mode-grid")
    );
  } catch {
    return false;
  }
...
class NexaTagsInstance {
  ...
  init() {
    ...
    wrapper.style.cssText = `
      ...
      min-height: 38px;
      padding: 4px 8px;
      border: 1px solid ${dm ? "#3d3d3d" : "#ddd"};
      border-radius: 4px;
      background: ${dm ? "#2c2c2c" : "white"};
      cursor: text;
      width: 100%;
    `;
    ...
    dropdown.style.cssText = `
      ...
      border: 1px solid ${dm ? "#3d3d3d" : "#ddd"};
      border-radius: 4px;
      box-shadow: 0 2px 8px rgba(0,0,0,${dm ? "0.35" : "0.1"});
      max-height: 200px;
      overflow-y: auto;
  ...
  searchAndShowSuggestions(query, dropdown) {
    ...
    // Render suggestions
    dropdown.innerHTML = "";
    const dmSuggest = nexaTagsIsDarkMode();
    ...
    if (isAtLimit) {
      ...
      limitMessage.style.cssText = dmSuggest
        ? `
        padding: 12px;
        text-align: center;
        color: #ffb74d;
    ...
    if (results.length > 0) {
      results.forEach((item, index) => {
        ...
        suggestionItem.addEventListener("mouseenter", () => {
          suggestionItem.style.backgroundColor = dmSuggest ? "rgba(63, 158, 255, 0.14)" : "#f5f5f5";
  ...
  highlightSuggestion(suggestions) {
    const hiBg = nexaTagsIsDarkMode() ? "rgba(63, 158, 255, 0.22)" : "#e3f2fd";
    suggestions.forEach((item, index) => {
      if (index === this.selectedIndex) {
        item.style.backgroundColor = hiBg;
